...
class User(AbstractUser):
    ...
    expenditure = models.DecimalField(max_digits=15, decimal_places=10, default=0.0000000000)  # to keep track of how much the user has spent
    email = models.EmailField(unique=True)
    questions_asked = models.IntegerField(default=0)
    question_asked_tokens = models.IntegerField(default=0)
    completion_tokens = models.IntegerField(default=0)
    total_tokens = models.IntegerField(default=0)
    relevant_pages = models.BooleanField(default = False)
    ...
    def save(self, *args, **kwargs):
        # If username is not provided, use a part of the email or a unique identifier
...
class Lecture (models.Model): 
    ...
    slides_text = models.JSONField(null = True, blank = True)
...
class Slide_Messages (models.Model): 
    user = models.ForeignKey('base.User', on_delete=models.CASCADE)
    course = models.ForeignKey(Course, on_delete=models.CASCADE)
    lecture = models.ForeignKey(Lecture, on_delete=models.CASCADE, null = True, blank=True)
    body = models.TextField()
